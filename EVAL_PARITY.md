...
| 9 | `294039c`: contradictory alignment modes rejected | **GATE — APPLIED** | `reject_contradictory_modes=False` fits each existing evaluator signature without a helper or restructure. The default preserves import-state precedence; explicit strict mode rejects contradictory flags. Strict mode is a library opt-in with no shipped in-repository caller. |

...
"""
import io, json, os, os.path as osp, shutil, subprocess, sys, tempfile, zipfile
import urllib.error
import yaml
...
ASSETS_ZIP = {
    ...
    "dslr_resized_mask_dir": ("dslr/resized_anon_masks",  ["DSC00001.png", "DSC00002.png"]),
    "iphone_colmap_dir":     ("iphone/colmap",            ["cameras.txt", "images.txt", "points3D.txt"]),
}
    ...
    for s in SCENES:
        for asset, (reldir, files) in ASSETS_ZIP.items():
            ...
            with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
                for f in files:
                    zf.writestr(f"{osp.basename(reldir)}/{f}", f"{s}:{reldir}/{f}".encode() * 40)
...
class FakeNet:
    ...
    def __init__(self, blobs): self.blobs, self.fail_at, self.n = blobs, None, 0
    def urlretrieve(self, url, filename):
        ...
        if self.fail_at is not None and self.n >= self.fail_at:
            ...
            with open(filename, "wb") as f: f.write(data[: len(data) // 2])
...
def make_cfg(data_root, keep_zipped):
    ...
    cfg.update(root_url="https://fake/download?token=TOKEN&filepath=FILEPATH",
               token="tok", data_root=data_root,
               ...
               exclude_assets={}, download_scenes=SCENES, verbose=False)
    cfg.pop("download_splits", None)
    p = osp.join(data_root, "_cfg.yml"); os.makedirs(data_root, exist_ok=True)
    yaml.safe_dump(cfg, open(p, "w")); return p
...
def verify(root):
    r = subprocess.run([sys.executable, osp.join(DL, "verify_scannetpp.py"), root,
                        "--config", osp.join(root, "_cfg.yml")],
...
def main():
    ...
    # ---- 1. keep_zipped: true ------------------------------------------
    print("\n1. keep_zipped: true")
    zr = osp.join(tmp, "zipped"); cfg = make_cfg(zr, True)
    net = FakeNet(blobs); run(cfg, net)
    ...
    check("archives NOT extracted", not osp.isdir(osp.join(zr, "data", SCENES[0], "dslr", "resized_images")))
    check("transport-zipped single file IS extracted",
          osp.isfile(osp.join(zr, "data", SCENES[0], "scans", "mesh_aligned_0.05.ply")))
    check("its archive is not left behind",
          not osp.isfile(osp.join(zr, "data", SCENES[0], "scans", "mesh_aligned_0.05.zip")))
    rc, out = verify(zr); check("verify_scannetpp passes", rc == 0, out.splitlines()[0])

    # ---- 2. keep_zipped: false (stock behaviour still works) -----------
    print("\n2. keep_zipped: false  (unchanged stock path)")
    er = osp.join(tmp, "extracted"); cfg2 = make_cfg(er, False)
    run(cfg2, FakeNet(blobs))
    e_inodes = count_inodes(er)
    check("archives extracted", osp.isdir(osp.join(er, "data", SCENES[0], "dslr", "resized_images")))
    check("zip deleted after extract", not osp.isfile(osp.join(er, "data", SCENES[0], "dslr", "resized_images.zip")))
    ...
    check("zip layout uses fewer inodes", z_inodes < e_inodes, f"{z_inodes} vs {e_inodes}")
    ...
    with open(victim, "r+b") as f: f.truncate(full // 2)
    rc, out = verify(zr); check("verify DETECTS truncation", rc != 0, out.splitlines()[0])
    net2 = FakeNet(blobs); run(cfg, net2)
    ...
    rc, out = verify(zr); check("verify passes again", rc == 0, out.splitlines()[0])

    # ---- 5. crash mid-transfer leaves no final-named file ---------------
    print("\n5. atomicity: job killed mid-transfer")
    cr = osp.join(tmp, "crash"); cfg3 = make_cfg(cr, True)
    net3 = FakeNet(blobs); net3.fail_at = 8  # crash after several assets have landed
    try: run(cfg3, net3)
    except KeyboardInterrupt: pass
    strays = [osp.join(dp, f) for dp, _, fs in os.walk(cr) for f in fs
              if f.endswith((".zip", ".mkv", ".ply"))]
    bad = [p for p in strays if p.endswith(".zip") and not _zip_ok(p)]
    check("no truncated file under a final name", not bad and len(strays) >= 5, f"{len(bad)} bad of {len(strays)} landed")
    ...
    sr = osp.join(tmp, "nosplit"); cfg4 = make_cfg(sr, True)
    import yaml as _y
    c = _y.safe_load(open(cfg4)); c["download_scenes"] = SCENES + ["deadbeef99"]
    _y.safe_dump(c, open(cfg4, "w"))
    try:
        run(cfg4, FakeNet(blobs)); raised = None
    except AssertionError as e:
        raised = str(e)
    check("raises AssertionError naming the scene",
          raised is not None and "deadbeef99" in raised, (raised or "no exception")[:70])
...
def _zip_ok(p):
    try:
        with zipfile.ZipFile(p): return True

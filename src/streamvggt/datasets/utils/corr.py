# Correspondence extraction utilities for the streamvggt dataset pipeline.
#
# Copied from DUSt3R (dust3r.datasets.utils.corr); the only changes are the
...
def extract_correspondences_from_pts3d(
    ...
    if n_positives < target_n_positives:
        raise ValueError(
            f"requested {target_n_positives} positive correspondences, "
            f"but only {len(pos1)} are available"
        )

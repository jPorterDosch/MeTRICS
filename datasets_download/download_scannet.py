...
def download_release(release_scans, out_dir, file_types, use_v1_sens, skip_existing):
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        futures = {executor.submit(_dl, s): s for s in release_scans}
        for i, future in enumerate(as_completed(futures)):
            scan_id, err = future.result()
            if err:
                print(f"FAILED: {scan_id}: {err}")
                failed.append(scan_id)
            print(f"Progress: {i + 1}/{len(release_scans)}")

    if failed:
        print(f"\n{len(failed)} scenes failed. Re-run with --skip_existing to retry:")
        for s in failed:
            print(" " + s)
...
def main():
    ...
    if args.v1:
        ...
        assert (not args.tf_semantic) and (not args.grit), (

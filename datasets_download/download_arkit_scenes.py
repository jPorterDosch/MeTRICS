...
def download_laser_scanner_point_clouds_for_video(video_id, metadata, download_dir):
    ...
    has_laser_scanner_point_clouds = video_metadata[
        "has_laser_scanner_point_clouds"
    ].iat[0]

    if not has_laser_scanner_point_clouds:
        print(
            f"Warning: Laser scanner point clouds for video {video_id} are not available"
        )
        return
...
def download_laser_scanner_point_clouds(
    ...
    for extension in [".ply", "_pose.txt"]:
        ...
        if os.path.exists(filepath):
            return
        file_url = (
            f"{ARkitscense_url}/raw/laser_scanner_point_clouds/{visit_id}/{filename}"
        )
